...
type ColorRaceChartConfigProps = {
  colorRaceData: [...[string, number]]
  widthScreen: number
  yAxisType: string
}


export const colorRaceChartConfig = ({ colorRaceData, widthScreen, yAxisType }: ColorRaceChartConfigProps) => {
  ...
  if(colorRaceData !== undefined){
    colorRaceData.forEach(arr => {
  ...
  const option = {
    ...
    xAxis: {
      ...
      zlevel: 2,
      minInterval: widthScreen >= 320 && widthScreen <= 768 ? 20000 : null
    },
    yAxis: {
      type: 'category',
